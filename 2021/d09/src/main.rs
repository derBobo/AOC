...
fn main() {
    ...
    let temp_map: Vec<Vec<u8>> = buffered
        .lines()
        .map(|y| {
            y.unwrap()
                .chars()
    ...
    let mut basins: Vec<u32> = low_points
        ...
        .map(|&x| {
            ...
            while points_to_check.len() != 0 {
                ...
                if map[current.0 + 1][current.1] != 9
                    && !points_to_check.contains(&(current.0 + 1, current.1))
                    && !checked_points.contains(&(current.0 + 1, current.1))
                {
                ...
                if map[current.0][current.1 + 1] != 9
                    && !points_to_check.contains(&(current.0, current.1 + 1))
                    && !checked_points.contains(&(current.0, current.1 + 1))
                {
